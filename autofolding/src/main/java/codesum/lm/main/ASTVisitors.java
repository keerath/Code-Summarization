...
public class ASTVisitors {
    ...
    public static class TreeCreatorVisitor extends ASTVisitor {
        ...
        protected static final HashSet<Integer> foldableTypes = new HashSet<Integer>(
                Arrays.asList(ASTNode.COMPILATION_UNIT, ASTNode.BLOCK,
        ...
        public HashMap<Range<Integer>, String> blockCommentFolds = Maps
                .newHashMap();
        public HashMap<Range<Integer>, String> lineCommentFolds = Maps
                .newHashMap();
        ...
        public void preVisit(final ASTNode node) {
            ...
            if (foldableTypes.contains(node.getNodeType()) && node.getLength() != 0) {
                ...
                if (node.getNodeType() == ASTNode.JAVADOC) {
                    ...
                    if (!jdoc.tags().isEmpty()) { // Handle empty comment
                        ...
                        final String firstTag = ((TagElement) jdoc.tags()
                                .get(0)).toString();
            ...
            if (node.getLength() != 0) {
                ...
                if (foldableTypes.contains(node.getNodeType())) {
                    ...
                    if (foldableStack.size() > 0) {
                        ...
                        if (node.getNodeType() == ASTNode.JAVADOC
                                && (node.getParent().getNodeType() == ASTNode.TYPE_DECLARATION || node
                                .getParent().getNodeType() == ASTNode.ENUM_DECLARATION)) {
        ...
        protected void shiftStartPosition(final ASTNode node) {
            ...
            if (((BodyDeclaration) node).getJavadoc() != null) {
                ...
                final String nodeString = fileString.substring(nodeStart,
                        nodeEnd);
                ...
                jdocEndIdx += 1;
                node.setSourceRange(nodeStart + jdocEndIdx, nodeEnd - nodeStart
                        - jdocEndIdx + 1);
        ...
        protected void addCommentsTree() {
            ...
            final HashMap<Integer, Range<Integer>> lineCommentRanges = Maps
                    .newHashMap();
            final HashMap<Integer, String> lineComments = Maps.newHashMap();

            // Get range (in chars) of comments
            final Table<Comment, ASTNode, ArrayList<String>> commentNodes = HashBasedTable
                    .create();
            for (final Comment node : (List<Comment>) cu.getCommentList()) {
                ...
                if (node.getNodeType() == ASTNode.BLOCK_COMMENT) {
                    ...
                    final Range<Integer> range = Range.closed(
                            node.getStartPosition(), node.getStartPosition()
                                    + node.getLength() - 1);
                    final String commentText = fileString.substring(
                            range.lowerEndpoint(), range.upperEndpoint());
                    blockCommentFolds.put(range,
                            commentText.replaceAll("/\\*|\\*/", "").trim()
                                    .split("\n")[0].replaceFirst("^\\*", ""));
                    allFolds.add(range);
                ...
                if (foldLineComments
                        && node.getNodeType() == ASTNode.LINE_COMMENT) {

                    final int line = cu.getLineNumber(node.getStartPosition());
                    lineRanges.add(Range.singleton(line).canonical(
                    ...
                    lineComments.put(line, commentText.replaceAll("//", "")
                            .trim());
                }
            ...
            if (foldLineComments) {
                conflate:
                for (final Range<Integer> lineRange : lineRanges
                        .asRanges()) {
                    ...
                    for (int line = startLine; line < endLine; line++) {
                        final Integer endLineChar = lineCommentRanges.get(line)
                                .upperEndpoint() + 1; // else lose last char
                        final Integer startNextLineChar = lineCommentRanges
                                .get(line + 1).lowerEndpoint();
                        final String inbetweenText = fileString.substring(
                                endLineChar, startNextLineChar).trim();
                        if (!inbetweenText.isEmpty())
                            continue conflate;
                    }

                    // Get character range for conflated comment
                    final Integer startChar = lineCommentRanges.get(startLine)
                            .lowerEndpoint();
                    final Integer endChar = lineCommentRanges.get(endLine)
                            .upperEndpoint();
                    final Range<Integer> range = Range.closed(startChar,
                            endChar);
        ...
        protected void addImportsTree() {
            ...
            @SuppressWarnings("unchecked")
            final List<ImportDeclaration> imports = (List<ImportDeclaration>) cu
                    .imports();
            if (!imports.isEmpty()) {

                // Get range of imports (starting after first import statement)
                final ImportDeclaration firstImport = imports.get(0);
                final int startChar = firstImport.getStartPosition()
                        + firstImport.getLength() - 1;
                final ImportDeclaration lastImport = imports
                        .get(imports.size() - 1);
                final int endChar = lastImport.getStartPosition()
                        + lastImport.getLength() - 1;
                ...
                for (final ImportDeclaration importNode : imports)
                    putTokenizedImports(importIdentifiers, importNode.getName()
                ...
                final ImportDeclaration node = cu.getAST()
         ...
         * Tokenize imports
         */
        private void putTokenizedImports(final List<String> importIdentifiers,
         ...
         */
        public ArrayList<String> tokenizeCommentString(
         ...
         */
        protected void putSplitToken(final List<String> identifierList,
                                     final String token) {
            if (splitTokens)
     ...
     */
    public static ASTNode getCoveringBlock(final CompilationUnit root,
                                           final ASTNode node) {

        final CoveringBlockFinderVisitor finder = new CoveringBlockFinderVisitor(
     ...
     */
    public static void putTokenParts(final List<String> identifierList,
                                     final String identifier) {
        for (final String token : identifier
